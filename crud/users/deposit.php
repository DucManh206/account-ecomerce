...
<body class="bg-nexus">
    ...
    <script>
        const currentDepositId = <?php echo $current ? intval($current['id']) : 0; ?>;
        ...
        async function pollDepositStatus() {
            ...
            try {
                ...
                fd.append('deposit_id', currentDepositId);
                const res = await fetch('/crud/api/deposit_poll.php', { method: 'POST', body: fd, credentials: 'same-origin' });
                const data = await res.json();
                if (!data.success) {
                    if (statusEl) statusEl.innerText = data.message || 'Không kiểm tra được giao dịch';
                    return;
                }
                depositRemaining = Number(data.remaining_seconds || 0);
                if (statusEl) statusEl.innerText = data.message || 'Đang chờ giao dịch';
                ...
                if (statusEl) statusEl.innerText = 'Lỗi kiểm tra, sẽ thử lại sau 3 giây';
            }
        }
        ...
        if (currentDepositId) {
            ...
            pollDepositStatus();
            depositCountdownTimer = setInterval(updateDepositCountdown, 1000);
            depositPollTimer = setInterval(pollDepositStatus, 3000);
        }
