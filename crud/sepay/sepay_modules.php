...
function sepay_apiCall($endpoint, $params = [], $method = 'GET') {
    ...
    // SePay limit: 3 calls / second. Project policy: poll every 3 seconds.
    // This file lock prevents accidental concurrent requests from multiple browser tabs.
    $lockFile = sys_get_temp_dir() . '/nexus_sepay_api.lock';
    $lockFp = fopen($lockFile, 'c+');
    if ($lockFp) {
        flock($lockFp, LOCK_EX);
        $last = floatval(trim(stream_get_contents($lockFp)) ?: 0);
        $now = microtime(true);
        $wait = 1.05 - ($now - $last);
        if ($wait > 0) usleep((int)($wait * 1000000));
        ftruncate($lockFp, 0);
        rewind($lockFp);
        fwrite($lockFp, (string)microtime(true));
        fflush($lockFp);
    ...
    if ($error) return ['error' => 'Lỗi cURL: ' . $error, 'http_code' => $httpCode];
    $data = json_decode($response, true);

    if ($httpCode === 429) {
        return ['error' => 'SePay rate limit: HTTP 429 Too Many Requests', 'http_code' => 429, 'raw' => $response];
    }
    if ($httpCode !== 200 || !isset($data['status']) || intval($data['status']) !== 200) {
        return ['error' => 'API Error', 'http_code' => $httpCode, 'message' => $data['messages'] ?? $data['error'] ?? 'Unknown error', 'raw' => $response];
    }
    return $data;
}
...
function sepay_getTransactions($params = []) {
    $config = sepay_getConfig();
    $defaults = ['limit' => 50];
    if (!empty($config['account_number'])) $defaults['account_number'] = $config['account_number'];
    return sepay_apiCall('transactions/list', array_merge($defaults, $params));
}

function sepay_normalizeText($text) {
...
function sepay_contentHasCode($content, $code) {
    $content = sepay_normalizeText($content);
    $code = sepay_normalizeText($code);
    if ($code === '') return false;
    $compactContent = preg_replace('/[^A-Z0-9]/', '', $content);
    $compactCode = preg_replace('/[^A-Z0-9]/', '', $code);
    return strpos($content, $code) !== false || strpos($compactContent, $compactCode) !== false;
}
...
function sepay_processTransaction($transactionId) {
    ...
    // Tìm deposit pending cùng số tiền amount_in và unique_code xuất hiện trong nội dung CK.
    // Không phụ thuộc format ngân hàng; chỉ cần 2 dữ liệu quan trọng: amount_in + transaction_content.
    ...
    while ($dep = mysqli_fetch_assoc($depResult)) {
        if (sepay_contentHasCode($content, $dep['unique_code'] ?? '')) {
            $matchedDeposit = $dep;
            break;
        }
    }
...
function sepay_syncAndProcess($limit = 100) {
    ...
    $params = ['limit' => $limit];
    // Chỉ cần lấy giao dịch gần đây để giảm dữ liệu; match dựa trên amount_in + transaction_content.
    $params['transaction_date_min'] = date('Y-m-d', time() - 86400);

    $response = sepay_getTransactions($params);
