...
if (!function_exists('products_primary_key')) {
function products_primary_key() {
    foreach (products_columns() as $name => $meta) if (($meta['Key'] ?? '') === 'PRI') return $name;
    return array_key_exists('id', products_columns()) ? 'id' : null;
}
}
if (!function_exists('products_getAll')) {
function products_getAll() {
    global $conn;
    $pk = products_primary_key();
    $order = $pk ? " ORDER BY `$pk` DESC" : '';
    $result = mysqli_query($conn, "SELECT * FROM `products`" . $order);
    return $result ? mysqli_fetch_all($result, MYSQLI_ASSOC) : [];
}
}
if (!function_exists('products_add')) {
function products_add($data) {
    global $conn;
    $cols = products_columns(); $fields=[]; $values=[];
